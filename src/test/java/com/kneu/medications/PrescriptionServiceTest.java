...
public class PrescriptionServiceTest {
  ...
  @MockitoBean PrescriptionRepository prescriptionRepository;

  @MockitoBean PrescriptionMapper prescriptionMapper;

  @MockitoBean MedicationRepository medicationRepository;
  ...
  void whenPrescriptionWithIdExistCorrectPrescriptionReturned() {
    ...
    when(prescriptionMapper.toDto(prescription))
        .thenReturn(
            new PrescriptionDto(
                prescription.getId(),
                prescription.getIntakeTime(),
                prescription.getLabel(),
  ...
  void whenPrescriptionInputValidPrescriptionCreatedSuccessfully() {
    ...
    assertEquals("Prescription id differs from asserted", PRESCRIPTION_ID, prescriptionDto.id());
    assertEquals(
        "Intake time differs from asserted", INTAKE_TIME, prescriptionDto.intakeTime().toString());
    assertEquals("Label differs from asserted", LABEL, prescriptionDto.label());
  }

  @Test
  void whenMissingMedicationIdPrescriptionIsNotCreated() {
    CreatePrescriptionRequest invalidRequest =
        new CreatePrescriptionRequest(null, INTAKE_TIME, LABEL);
    assertThrows(
        IllegalArgumentException.class,
        () -> prescriptionService.createPrescription(invalidRequest));
  }

  @Test
  void whenMissingIntakeTimePrescriptionIsNotCreated() {
    CreatePrescriptionRequest invalidRequest =
        new CreatePrescriptionRequest(MEDICATION_ID.toString(), null, null);
    assertThrows(
        IllegalArgumentException.class,
        () -> prescriptionService.createPrescription(invalidRequest));
  }
  ...
  void whenIntakeTimeInvalidPrescriptionIsNotCreated() {
    CreatePrescriptionRequest invalidRequest =
        new CreatePrescriptionRequest(MEDICATION_ID.toString(), "10", null);
    assertThrows(
        IllegalArgumentException.class,
        () -> prescriptionService.createPrescription(invalidRequest));
  }
  ...
  void whenMedicationNotFoundPrescriptionIsNotCreated() {
    CreatePrescriptionRequest request =
        new CreatePrescriptionRequest(MEDICATION_ID.toString(), INTAKE_TIME, null);

    when(medicationRepository.findById(MEDICATION_ID)).thenReturn(Optional.empty());
    assertThrows(
        EventProcessingException.class, () -> prescriptionService.createPrescription(request));
  }
